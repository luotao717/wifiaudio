...
# $Id: config-vlan.sh,v 1.39 2011-03-28 10:01:13 kurtis Exp $
#
...
usage()
{
	...
	echo "  $0 3 0 - restore Ralink RT6855 ESW to no VLAN partition"
	echo "  $0 3 LLLLW - config Ralink RT6855 ESW with VLAN and WAN at port 4"
	echo "  $0 3 WLLLL - config Ralink RT6855 ESW with VLAN and WAN at port 0"
	echo "  $0 3 12345 - config Ralink RT6855 ESW with VLAN 1~5 at port 0~4"
	echo "  $0 3 GW - config Ralink RT6855 ESW with WAN at Giga port"
	exit 0
}
...
config6855Esw()
{
	...
	if [ "$1" = "LLLLW" ]; then
		...
		#VLAN member port
		switch vlan set 6 7 1111011
		switch vlan set 7 8 0000101
		else
		#set PVID
		...
		switch reg w 2514 10007 #port5
		#VLAN member port
		switch vlan set 6 7 0111111
		switch vlan set 7 8 1000001
		else
		#set PVID
