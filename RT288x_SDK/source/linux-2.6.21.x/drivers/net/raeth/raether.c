...
static inline int rt2880_eth_send(struct net_device* dev, struct sk_buff *skb, int gmac_no)
{
	...
	if(skb_shinfo(skb)->nr_frags > 0) { 
		ei_local->tx_ring0[tx_cpu_owner_idx0].txd_info4.TSO = 1;
	...
	if( skb_shinfo(skb)->gso_segs > 1) {
		...
		if(iph->protocol == IPPROTO_TCP) {
				...
				printk("IP: tot_len=%d id=0x%x protocol=%x\n", ntohs(iph->tot_len), ntohs(iph->id), iph->protocol);
...
int ei_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	esw_reg reg;
#endif
#if defined(CONFIG_RALINK_RT3352) || defined(CONFIG_RALINK_RT5350) || defined (CONFIG_RALINK_RT6855) || defined(CONFIG_RALINK_RT63365)
        esw_rate ratelimit;
#endif
...
#define _ESW_REG(x)	(*((volatile u32 *)(RALINK_ETH_SW_BASE + x)))
		...
		case RAETH_ESW_PHY_DUMP:
			...
			for(offset=0;offset<=16;offset++) {
			    ...
			    mii_mgr_read(0,offset, &value);
			    printk("%02d: %04X ",offset, value);
			}

			if (reg.val ==32 ) {//dump all phy register
			    for(offset=0;offset<4;offset++) { //global register  page 0~4
